...
docker run --rm -ti -v ~/.netrc:/workdir/.netrc:ro -v ${LOCAL_DATA_DIR}:${DOCKER_DATA_DIR}:ro -v ${SCRIPT_DIR}:${DOCKER_CODE_DIR} -v ${LOCAL_PRODUCT_DIR}:${DOCKER_PRODUCT_DIR} -e DOCKER_SHAPEFILE_PATH=${DOCKER_SHAPEFILE_PATH} hyp3_timeseries:latest /hyp3_timeseries/run.sh
